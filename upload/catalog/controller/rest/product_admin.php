...
class ControllerRestProductAdmin extends RestAdminController
{
    ...
    public function products()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (isset($this->request->get['id']) && $this->isInteger($this->request->get['id'])) {
                ...
                if (isset($this->request->get['category']) && $this->isInteger($this->request->get['category'])) {
    ...
    private function getProductInfo($product)
    {
        ...
        $stores = $this->model_rest_restadmin->getProductStores($product['product_id']);
        ...
        $ret = array(
            ...
            'discounts' => $discounts,
            'options' => $options,
            'minimum' => isset($product['minimum']) ? (int)$product['minimum'] : 1,
            'upc' => $product['upc'],
            'ean' => $product['ean'],
            'jan' => $product['jan'],
            'isbn' => $product['isbn'],
            ...
            'manufacturer_id' => !empty($product['manufacturer_id']) ? (int)$product['manufacturer_id'] : "",
            ...
            'reward' => isset($product['reward']) ? (int)$product['reward'] : '',
        ...
        $ret['currency_id'] = (int)$this->currency->getId($this->config->get('config_currency'));
    ...
    public function listProducts($category_id, $request)
    {
        ...
        $total = $this->model_rest_restadmin->getProductsData($parameters, null, true);
        ...
        if (isset($data['product_seo_url'])) {
            ...
            foreach ($data['product_seo_url'] as &$keywordData) {
                ...
                if (!empty($keywordData["keyword"])) {
                    ...
                    if (!empty($current)) {
                        $data['product_id'] = $current['product_id'];
                    }
    ...
    private function deleteProducts($products)
    {
        ...
        foreach ($products as $productItem) {
            ...
            $this->error = array();
            $id = (int)$productItem['product_id'];

            $product = $this->model_rest_restadmin->checkProductExists($id);

            if ($product) {
                $validProducts[$id] = $productItem;
            } else {
                $productItem['error'][] = "Product not found";
                $errors[] = $productItem;
            }
        }

        if (empty($errors)) {
            foreach ($validProducts as $id => $validProduct) {
                $this->model_rest_restadmin->deleteProduct($id);
            }
        } else {
    ...
    private function updateProducts($products)
    {
        ...
        if (empty($errors)) {
            foreach ($validProducts as $id => $validProduct) {
                $this->model_rest_restadmin->editProductById($id, $validProduct);
            }
        } else {
            $this->json['error'] = $errors;
            $this->statusCode = 400;
    ...
    public function featured()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (isset($this->request->get['limit']) && $this->isInteger($this->request->get['limit']) && $this->request->get['limit'] > 0) {
    ...
    public function productbymodel()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (isset($this->request->get['model'])) {
                $product = $this->model_rest_restadmin->getProductByModel($this->request->get['model']);
                if (empty($product)) {
                    $this->json['error'][] = 'Product not found';
                    $this->statusCode = 404;
                } else {
            ...
            if (isset($this->request->get['model'])) {
                ...
                if (empty($product)) {
                    ...
                    $this->updateProduct($product['product_id'], $post);
                }
            } else {
                $this->json['error'][] = 'Invalid model.';
                $this->statusCode = 400;
            }
        } else if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    public function getproductidbyparameter()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (
                isset($this->request->get['value']) && !empty($this->request->get['value'])
                && isset($this->request->get['p']) && !empty($this->request->get['p'])
            ) {
                $value = trim($this->request->get['value']);
                $param = trim($this->request->get['p']);

                if ($param == 'ean') {
                    $result = $this->model_rest_restadmin->getProductIdByEan($value);
                } elseif ($param == 'isbn') {
                    $result = $this->model_rest_restadmin->getProductIdByISBN($value);
                } elseif ($param == 'sku') {
                    $result = $this->model_rest_restadmin->getProductIdBySKU($value);
                } elseif ($param == 'model') {
                    $result = $this->model_rest_restadmin->getProductIdByModel($value);
                } else {
                    $result = array();
                }
                ...
                $this->statusCode = 400;
                $this->json['error'][] = "Missing GET parameter";
            }
        } else {
            $this->statusCode = 405;
            $this->allowedHeaders = array("GET");
