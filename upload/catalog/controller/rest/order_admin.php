...
class ControllerRestOrderAdmin extends RestAdminController
{
    ...
    public function getOrder($order_id)
    {
        ...
        if ($this->isInteger($order_id)) {
            $order_info = $this->model_rest_restadmin->getOrder($order_id);
    ...
    private function getOrderDetailsToOrder($order_info)
    {
            ...
            foreach ($products as $product) {
                ...
                foreach ($options as $option) {
                    if ($option['type'] != 'file') {
                        $option_data[] = array(
                            'name' => $option['name'],
                            'value' => isset($option['real_value']) ? $option['real_value'] : $option['value'],
                            'type' => $option['type'],
                            'product_option_id' => isset($option['real_product_option_id']) ? (int)$option['real_product_option_id'] : "",
                            'product_option_value_id' => isset($option['product_option_value_id']) ? (int)$option['product_option_value_id'] : "",
                            'option_id' => isset($option['real_option_id']) ? (int)$option['real_option_id'] : "",
                ...
                $orderData['products'][] = array(
                    ...
                    'tax_class_id' => (int)$origProduct['tax_class_id'],
        ...
        $shippingTaxClassId = $this->config->get('shipping_'.$shippingCode[0] . '_tax_class_id');
        ...
        $orderData['shipping_lines'] = array();
        ...
        if (!empty($shippingTaxClassId)) {
            ...
            foreach ($shipping_lines as $line) {
                ...
                if ($line['type'] == "P") {
                    ...
                    $taxValue = ($shipping_total / 100) * $rate;

                    $tax = array(
                        'name' => $line['name'],
                        'price' => $this->currency->format($shipping_total, $order_info['currency_code'], $order_info['currency_value'], false),
                        'tax' => $this->currency->format($taxValue, $order_info['currency_code'], $order_info['currency_value'], false)
                    );

                    $orderData['shipping_tax'] += $taxValue;
                } else {
                    $tax = array(
                        'name' => $line['name'],
                        'price' => 0,
        ...
        $orderData['shipping_exclude_tax'] = (float)$this->currency->format($shipping_total, $order_info['currency_code'], $order_info['currency_value'], false);
        ...
        //get ID of shipping points for hungarian providers
        $orderData['atvpont'] = "";
        $shippingMethod = mb_strtolower($orderData['shipping_method']);

        if (mb_strpos($shippingMethod, "pont") !== false ||
            mb_strpos($shippingMethod, "pickpack") !== false ||
            mb_strpos($shippingMethod, "foxpost") !== false
        ) {
            $orderData['atvpont'] = filter_var($orderData['shipping_code'], FILTER_SANITIZE_NUMBER_INT);
        }

        if (isset($orderData['custom_field']) && !empty($orderData['custom_field'])) {
            foreach ($orderData['custom_field'] as $cId => $customValue){
                if (filter_var(trim($customValue), FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => '/[0-9]{8}-[0-9]{1}-[0-9]{2}/')))){
                    $orderData['hungarian_vat_number'] = trim($customValue);
                    break;
                }
            }
        }
    ...
    public function listOrders()
    {
        ...
        if (isset($this->request->get['limit']) && $this->request->get['limit'] != "" && $this->isInteger($this->request->get['limit'])) {
        ...
        if (isset($this->request->get['offset']) && $this->request->get['offset'] != "" && $this->isInteger($this->request->get['offset'])) {
            $page = $this->request->get['page'];
        }
        ...
        if (isset($this->request->get['filter_id_lower_than']) && $this->isInteger($this->request->get['filter_id_lower_than'])) {
        ...
        if (isset($this->request->get['filter_id_larger_than']) && $this->isInteger($this->request->get['filter_id_larger_than'])) {
        ...
        $total = $this->model_rest_restadmin->getAllOrders($data, true);
    ...
    public function updateOrder($id, $data)
    {
        ...
        if ($this->isInteger($id)) {
            ...
            if (isset($data['status']) && $this->isInteger($data['status'])) {
    ...
    public function listorderswithdetails()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $total = $this->model_rest_restadmin->getOrdersByFilter($data, true);
    ...
    public function userorders()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            if (isset($this->request->get['user']) && $this->request->get['user'] != ""
                && $this->isInteger($this->request->get['user'])) {
