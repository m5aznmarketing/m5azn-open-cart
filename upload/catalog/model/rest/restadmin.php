...
class ModelRestRestAdmin extends Model {
    ...
    public function getCustomers($data = array(), $onlyCount=false) {
        if($onlyCount) {
            $sql = "SELECT COUNT(*) as total ";
        } else {
            $sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name,c.firstname as firstname,c.lastname as lastname,  cgd.name AS customer_group ";
        }

        $sql.= "
              FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
    ...
    public function getCustomerGroups($data = array() , $onlyCount=false) {
        if($onlyCount) {
            $sql = "SELECT COUNT(*) as total";
        } else {
            $sql = "SELECT * ";
        ...
        $sql.= " FROM " . DB_PREFIX . "customer_group cg LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (cg.customer_group_id = cgd.customer_group_id)";
    ...
    public function editCustomer($customer_id, $data) {
        ...
        //fix opencart affiliate custom field handling
        if (isset($data['affiliate'])) {
            if (!isset($data['affiliate']['custom_field'])) {
                $data['affiliate']['custom_field'] = array();
            }
            $temp = array();
            ...
            if(!empty($data['affiliate']['custom_field'])) {
                foreach ($data['affiliate']['custom_field'] as $key => $value) {
                    $temp[$key] = $value;
                }
            }

            $data['custom_field'] = $temp;
        }

        $data['custom_field'] = isset($data['custom_field']) ? json_encode($data['custom_field']): array();
    ...
    public function getCategories($parent_id = 0, $onlyCount = false, $data = array()) {
        ...
        if($this->config->has('config_rest_store_id')){
            $sql.= " LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) ";
            $sql.= " WHERE  c2s.store_id = '" . (int)$this->config->get('config_rest_store_id') . "' ";
    ...
    public function addCategory($data) {
        ...
        if (isset($data['image'])) {
            $data['image'] = $this->downloadImage($data['image'], 'category');
    ...
    public function editCategory($category_id, $data) {
        ...
        if( isset( $data['category_description'] ) && !empty( $data['category_description'] ) ){
            foreach ( $data['category_description'] as $value ) {
                ...
                WHERE category_id = '" . (int)$category_id . "' AND language_id = '" . (int)$value['language_id'] . "' ");

                if (!$query->num_rows) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$value['language_id'] . "',
    ...
    public function editProductById($product_id, $data) {
        ...
        $data["image"] = $this->downloadImage($data["image"]);
    ...
    public function downloadImage($image, $subfolder = 'product', $parentId='') {

        if(!empty($image)) {

            $remoteFile = filter_var($image, FILTER_VALIDATE_URL);

            if ($remoteFile || file_exists($image)) {

                if (!file_exists(DIR_IMAGE . 'catalog/' . $subfolder)) {
                    mkdir(DIR_IMAGE . 'catalog/' . $subfolder, 0777, true);
                }
                ...
                $dbFilename = 'catalog/' . $subfolder . $filename;

                try {
                    if ($remoteFile && $this->validateImage($image, true)) {
                        $image = file_get_contents($image);
                        file_put_contents(DIR_IMAGE . 'catalog/' . $subfolder . $filename, $image);
                    } elseif (!$remoteFile && $this->validateImage($image)) {
                        copy($image, DIR_IMAGE . 'catalog/' . $subfolder . $filename);
                    } else {
                        $dbFilename = '';
                    }
                } catch (Exception $e) {
    ...
    public function getProductsData($data = array(), $customer=null, $onlyCount=false) {
    ...
    public function getAllOrders($data, $onlyCount=false) {
        if($onlyCount) {
            $sql = "SELECT COUNT(o.order_id) as total ";
        } else {
            $sql = "SELECT o.order_id, o.firstname, o.lastname, os.name as status,
    ...
    public function getOrdersByFilter($data = array(), $onlyCount=false) {
        if($onlyCount) {
            $sql = "SELECT COUNT(o.order_id) as total  ";
        } else {
        ...
        if($onlyCount) {
            ...
            foreach ($orders_query->rows as $order) {
                ...
                $orders[$index] = array(
                    ...
                    'payment_custom_field'    => json_decode($order['payment_custom_field'], true),
                    ...
                    'shipping_custom_field'   => json_decode($order['shipping_custom_field'], true),
                    'custom_field'   => isset($order['custom_field']) ? json_decode($order['custom_field'], true) : array(),
    ...
    public function getOrderOptions($order_id, $order_product_id) {
        $query = $this->db->query("
                    SELECT *, oo.product_option_id as real_product_option_id, po.option_id as real_option_id, oo.value as real_value FROM " . DB_PREFIX . "order_option as oo
                    LEFT JOIN " . DB_PREFIX . "product_option_value pov ON (oo.product_option_value_id = pov.product_option_value_id)
                    LEFT JOIN " . DB_PREFIX . "product_option po ON (po.product_option_id = oo.product_option_id)
    ...
    public function addManufacturer($data) {
        ...
        if (isset($data['image'])) {
            $data['image'] = $this->downloadImage($data['image'], 'manufacturer');
    ...
    public function getManufacturers($data = array(), $onlyCount=false) {
        if($onlyCount) {
            $sql = "SELECT COUNT(*) as total ";
        } else {
            $sql = "SELECT * ";
        }
        $sql.= " FROM " . DB_PREFIX . "manufacturer";
...
//    }
    ...
    public function isOptionValueNameExist($option_id, $option_name, $language_id) {
        ...
        $query = $this->db->query("SELECT  COUNT(*) AS total FROM `" . DB_PREFIX . "option_value_description` od
        WHERE od.language_id = '" . (int)$language_id . "'
        AND od.option_id = '" . (int)$option_id . "'  AND od.name = '" . $this->db->escape($option_name) . "'");
        ...
        if($optionSkuEnabled){
            ...
            if(!empty($query->row['product_id'])) {
                ...
                if ($product_query->row['product_id']) {
                    $result = $this->getProductsByIds(array($product_query->row['product_id']), null);
                }
            }
        }
    ...
    public function getProductStores($product_id) {
        ...
        if ($query->num_rows) {
            foreach ($query->rows as $result) {
                $product_store_data[] = (int)$result['store_id'];
            }
        }
    ...
    public function getAffliateByTracking($tracking) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_affiliate WHERE tracking = '" . $this->db->escape($tracking) . "'");

        return $query->row;
    }

    private function validateImage($file, $isRemote=false)
    {
        ...
        if ($valid) {
            ...
            if (!in_array(strtolower(substr(strrchr($file, '.'), 1)), $allowed)) {
                $valid = false;
            }
        }
    ...
    public function addFilter($data) {
        ...
        foreach ($data['filter_group_description'] as $value) {
            ...
            $this->db->query("INSERT INTO " . DB_PREFIX . "filter_group_description SET filter_group_id = '" . (int)$filter_group_id . "', language_id = '" . (int)$value['language_id'] . "', name = '" . $this->db->escape($value['name']) . "'");
        }
        ...
        if (isset($data['filter'])) {
            ...
            foreach ($data['filter'] as $filter) {
                ...
                foreach ($filter['filter_description'] as $filter_description) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "filter_description SET filter_id = '" . (int)$filter_id . "', language_id = '" . (int)$filter_description['language_id']  . "', filter_group_id = '" . (int)$filter_group_id . "', name = '" . $this->db->escape($filter_description['name']) . "'");
                }
            }
        }
    ...
    public function getProductIdByEan($ean) {
        $query = $this->db->query("SELECT product_id as id FROM " . DB_PREFIX . "product 
        WHERE LCASE(ean) = '" . $this->db->escape(utf8_strtolower($ean)) . "' LIMIT 1");
    ...
    public function getProductIdByISBN($isbn) {
        $query = $this->db->query("SELECT product_id as id FROM " . DB_PREFIX . "product 
        WHERE LCASE(isbn) = '" . $this->db->escape(utf8_strtolower($isbn)) . "' LIMIT 1");
    ...
    public function getProductIdBySKU($sku) {
        $query = $this->db->query("SELECT product_id as id FROM " . DB_PREFIX . "product 
        WHERE LCASE(sku) = '" . $this->db->escape(utf8_strtolower($sku)) . "' LIMIT 1");
    ...
    public function getProductIdByModel($model) {
        $query = $this->db->query("SELECT product_id as id FROM " . DB_PREFIX . "product 
        WHERE LCASE(model) = '" . $this->db->escape(utf8_strtolower($model)) . "' LIMIT 1");
    ...
    public function getCategoryByName($name) {
        $sql = "SELECT DISTINCT c.category_id as id FROM " . DB_PREFIX . "category c 
                    LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) ";

        if($this->config->has('config_rest_store_id')){
            $sql.= " LEFT JOIN " . DB_PREFIX . "category_to_store c2s ON (c.category_id = c2s.category_id) ";
            $sql.= " WHERE LCASE(cd.name) = '" . $this->db->escape(utf8_strtolower($name)). "' AND c2s.store_id = '" . (int)$this->config->get('config_rest_store_id') . "' ";
        } else {
            $sql.= " WHERE LCASE(cd.name) = '" . $this->db->escape(utf8_strtolower($name)) . "' ";
        }
